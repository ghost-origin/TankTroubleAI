/* probe_runtime6.js —— 全方向扇形测试: 游戏物理每方向走 0.5s, 与 raw_wall footprint 模拟对比 */
const fs = require('fs');
...
(async () => {
  ...
  for (let adeg = 0; adeg < 360; adeg += STEP) {
    ...
    results.push({ adeg, dist, realDir });
    console.log('dir=' + adeg + 'deg -> moved=' + dist.toFixed(1) + 'px (实际方向 ' + realDir.toFixed(0) + '°、end=(' +
      (me0.x - 197.5).toFixed(1) + ',' + (me0.y - 31).toFixed(1) + '))');
  }
  fs.writeFileSync(path.join(__dirname, 'fan_results.json'), JSON.stringify(results));
  process.exit(0);
